...
Go言語（Golang）の世界へようこそ！ Goはシンプルで高速な処理が得意な言語なので、ネットワークツールの作成には最適です。
...
# ヒント
- 【 A 】: `NewClient` - スピードテスト用の新しいクライアントを作成する関数です。
- 【 B 】: `DownloadTest` - ダウンロード速度を測定するメソッドです。
- 【 C 】: `UploadTest` - アップロード速度を測定するメソッドです。
